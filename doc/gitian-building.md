...
*Setup instructions for a Gitian build of Bitesax Core using a Debian VM or physical system.*
...
- [Building Bitesax Core](#building-bitesax-core)
...
Clone the git repositories for bitesax and Gitian.
...
in the bitesax repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bitesax Core
----------------

To build Bitesax Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitesax repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/bitesax-project/bitesax
...
For example:
```bash
URL=https://github.com/thrasher-/bitesax.git
...
and the bitesax git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitesax/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitesax
SIGPATH=/some/root/path/bitesax-detached-sigs

./bin/gbuild --url bitesax=${BTCPATH},signature=${SIGPATH} ../bitesax/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitesax-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitesax-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitesax-osx-build.assert
```
...
[bitesax-project/gitian.sigs.ltc](https://github.com/bitesax-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
